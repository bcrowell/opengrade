...
=head1 SYNOPSIS
  ...
  # Change the password of a preexisting file:
  $mac->rekey(INFILE=>"foo",OUTFILE=>"bar");
...
Aj=F(j," ",Lj),
F(x)=H(r,x),
H(x)=E_K(x) is a MAC function defined by doing p
iterations of the function SHA1(K,x), K is the key,
and Bn is the final member of the sequence defined by
sequence B0=F(""), Bj=F(Bj-1,j," ",Lj). The purpose
of choosing a large value of p is to
make dictionary attacks more computationally expensive. (In version
1 of the method, p was always 2.)

In the following example, a two-line text file has had line-by-line
authentication added to it, with password "secret":

  #,LineByLine.pm,207906,SHA1,2,50
...
In the first line, the number 207906 is the salt r, 2 gives the
version number of the authentication method, and 50 is the number
of SHA1 iterations p.
...
HASH_FUNCTION defaults to "SHA1", but "Whirlpool" is also legal.
Whatever the hash function is, I'm assuming it returns base64.

MIN_LINE_LENGTH defaults to 140. For readability, short lines get
...
Ben Crowell, www.lightandmatter.com
...
sub get_named_hash_function {
    ...
    if ($name eq "Whirlpool") {
          $result = sub {
            ...
            my $x = $data;
            for (my $i=1; $i<=$iterations-1; $i++) {
              $x = do_whirlpool($key.$x);
            }
            return do_whirlpool($key.$x);
          }
    }
    $self->{HASH_FUNCTION_REF} = $result;

}

# The following three subs are only here for legacy support of old format, and is duplicated in GradeBook.pm.
...
sub check_authenticity {
    ...
    if ($head =~ m/^([^,]*),([^,]*),([^,]*),([^,]*),([^,]*),([^,]*)/) {
      ...
      $self->{ITERATIONS} = 2; # reading an old file, where p was always 2
    }
    $self->get_named_hash_function();
...
# This implements the function F described in the documentation. It calls
# the function referred to by HASH_FUNCTION_REF, which is H in the
# documentation.
sub do_hash {
    my $self = shift;
    my $data = shift;
    my $the_function = $self->{HASH_FUNCTION_REF};
    return &$the_function($self->{SALT}.$data,$self->{KEY},$self->{ITERATIONS});
}
...
sub rekey {
    ...
    my %args = (
      INFILE=>"-",
      OUTFILE=>"-",
      TEMPFILE=>"LineByLine_temp_$$", # so we can do it in place
      @_
    );
    local ($_);

    open(INFILE,"<".$args{INFILE}) or return $!;
    open(TEMPFILE,">".$args{TEMPFILE}) or return $!;

    print TEMPFILE $self->head() or return "failed to print header";
    while (my $line = <INFILE>) {
      print TEMPFILE $self->line($self->strip_line($line)) or return "failed to write line";
    }
    print TEMPFILE $self->tail() or return "failed to write tail";

    close(INFILE) or return "failed to close input file";
    close(TEMPFILE) or return "failed to close temp file";
    unlink($args{OUTFILE}) or return "failed to delete old outfile";
    rename($args{TEMPFILE},$args{OUTFILE}) or return "failed to rename temporary file";
    return "";

}

# If the file isn't actually watermarked, this detects it and doesn't mess with it.
sub strip {
    my $self = shift;
    my %args = (
      INFILE=>"-",
    ...
    my $line = <INFILE>; # Header line goes in the bit bucket, if this really has a watermark.
    unless ($line =~ / \#,LineByLine.pm/) {close INFILE; return ""}

    open(TEMPFILE,">".$args{TEMPFILE}) or return $!;
    while (my $line = <INFILE>) {
      $line = $self->strip_line($line);
      if ($line ne "") {print TEMPFILE $line}
    }
